...
function OnSelect(num) {
	...
	if (rings[nowselect][1][num]) {
		GameEvents.SendToServerEnsured("chat_wheel:select_vo", { num: newnum });
	} else {
		$("#PhrasesContainer").RemoveAndDeleteChildren();
		for (let i = 0; i < 8; i++) {
...
function AddOnFavourites(num) {
	...
	GameEvents.SendToServerEnsured("chat_wheel:save", { favourites: favourites });
}
...
(function () {
	...
	GameUI.Player.RegisterForPlayerDataChanges(() => {
		favourites = Object.values(GameUI.Player.GetSettingValue("chat_wheel_favs") || {});
		UpdateFavourites();
	});
